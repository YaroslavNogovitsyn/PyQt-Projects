...
# запоминаем, что в форме редактирования вопроса с чем связано
frm_edit = QuestionEdit(0, txt_Question, txt_Answer, txt_Wrong1, txt_Wrong2, txt_Wrong3)
# список виджетов, который надо перемешивать (для случайного размещения ответов)
...
def show_random():
    ...
    show_question()  # показать на форме панель вопросов
...
def add_form():
    ...
    # получаем объект класса QModelIndex, который соответствует последнему элементу в данных
    ...
    # переводим фокус на поле редактирования вопроса, чтобы сразу убирались "болванки"
...
def connects():
    ...
    # при нажатии на элемент списка будет открываться для редактирования нужный вопрос
