...
class AnswerCheck(QuestionView):
    ...
    def check(self):
        ...
        if self.answer.isChecked():
            ...
            self.show_correct(text_correct)
        else:
            ...
            if self.wrong_answer1.isChecked() or self.wrong_answer2.isChecked() or self.wrong_answer3.isChecked():
                self.show_correct(text_wrong)  # надпись "верно" или "неверно"

    def show_correct(self, res):
        """ показать результат - установим переданный текст в надпись "результат" и покажем нужную панель """
        self.result.setText(res)  # надпись "верно" или "неверно"
...
class QuestionListModel(QAbstractListModel):
    ...
    def insertRows(self, parent=QModelIndex()):
        ...
        # вставка данных должна быть после этого указания и перед endInsertRows()
    ...
    def removeRows(self, position, parent=QModelIndex()):
        ...
        # сообщаем, что мы собираемся удалять строку - от position до position
