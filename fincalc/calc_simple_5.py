""" Калькулятор NPV, версия с улучшенным интерфейсом: 
используется QScrollArea, чтобы прокручивать строки

Класс CashFlow, в методе lay_widgets() строка 
        self.layout_main.addLayout(self.layout_famounts) 
меняется на вызов метода add_scroll().

Этот метод использует макет layout_famounts для виджета widget_scroll, и уже этот виджет добавляет в layout_main
...
class CashFlow(QWidget):
    ...
    def add_scroll(self):
        """ размещает таблицу с элементами денежного потока на область скроллинга, 
        т.е. её можно будет "проматывать", если содержимое перстанет вмещаться """
        area = QScrollArea()
