""" В опроснике используются списки: вопросов и профессий.
Одна группа с переключателями - это один объект, напишем такой класс.
Далее создадим список таких объектов и будем работать с ним.   
Объем кода уменьшается благодаря структурам данных.
Интерфейс пока тот же.

Создание группы написано практически теми же командами, что и в программе psytest_simple, 
но в конструкторе __init__ класса AlternativeGroup
(строки 61 - 68)
в этой версии пока нет нового варианта show_result
программа запускается так же, как и psytest_simple (см. в конце кода)
"""
