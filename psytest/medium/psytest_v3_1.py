...
Для показа одной альтернативы используется сложный виджет AllQuestions
- он хранит список из "панелек", они создаются все, но показывается только одна
(строка 79) 
в его конструктор переносится код создания списка groups, причем список является свойством объекта,
с ним нужно дальше работать
метод show_curr показывает "текущую панель", используя свойство "текущий номер" - current_choice
(строки 99 - 101)
основное окно теперь содержит один виджет AllQuestions и две кнопки под ним
(строка 139 и далее)
в этой версии пока не работают кнопки перехода
"""
